...
void Player::update() {
  //bool doConcludeBackTrack = false;
    ...
    //WARNING : For large mazes this still needs improvements to work correctly

    //Pros : No more teleportation
      
    //Idea : Instead of storing every cell visited, recreate the visited cells in a more efficient manner while also storing every unique cell. In the ideal case, we store the "Shortest All-Inclusive Path" ; i.e. storing all cells visited in such a manner two consecutive cells are interconnected and we provide the shortest path for the player to move in such that they visit every unique cell
    ...
    //For large mazes this is still highly inefficient because we are not using efficient pathing
